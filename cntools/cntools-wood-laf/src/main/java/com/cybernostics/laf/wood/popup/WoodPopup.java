...
public class WoodPopup extends Popup
{
		...
		Popup popup )
	{
		...
		( (JComponent) parentContainer ).putClientProperty(
		...
		if (this.heavyContainer != null && System.getProperty(
			"os.name" )
			.startsWith(
				"Mac" ))
		{
			...
			{
				( (JWindow) this.heavyContainer ).getRootPane()
					.putClientProperty(
	...
	private void fixCursorForInternalFrame( Component owner )
	{
		if (( owner != null ) && ( owner instanceof JInternalFrame ))
		{
			...
			{
				( (JFrame) topContainer ).getGlassPane()
					.setCursor(
						cursor );
				( (JFrame) topContainer ).getGlassPane()
					.setVisible(
				...
				if (topContainer instanceof JWindow)
				{
					( (JWindow) topContainer ).getGlassPane()
						.setCursor(
							cursor );
					( (JWindow) topContainer ).getGlassPane()
						.setVisible(
					...
					if (topContainer instanceof JDialog)
					{
						( (JDialog) topContainer ).getGlassPane()
							.setCursor(
								cursor );
						( (JDialog) topContainer ).getGlassPane()
							.setVisible(
						...
						if (topContainer instanceof JApplet)
						{
							( (JApplet) topContainer ).getGlassPane()
								.setCursor(
									cursor );
							( (JApplet) topContainer ).getGlassPane()
								.setVisible(
	...
	private void captureBackground()
	{
		if (this.heavyContainer != null)
		{
			try
			{
				...
				final Rectangle localRectangle = new Rectangle( this.x,
					this.y,
