...
//
// ################################################################################
package com.cybernostics.laf.wood.painter;

import com.cybernostics.laf.wood.popup.WoodComboBoxPopupMenuListener;
import com.cybernostics.laf.wood.popup.WoodPopup;
import com.cybernostics.laf.wood.popup.WoodPopupFactory;
import com.cybernostics.laf.wood.utils.WoodGraphicsUtils;
import java.awt.*;
...
/**
 * The painter class for Wood Look And Feel
 * 
 * @author EASYNTH
 */
public class WoodPainter extends SynthPainter
{
	 ...
	 */
	public static void gradientFillRect( final Graphics g,
		final int x,
		final int y,
		final int w,
		final int h,
	 ...
	 */
	@Override
	public void paintButtonBorder( final SynthContext context,
		final Graphics g,
									final int x,
		final int y,
		final int w,
		final int h )
	{
		...
		final int arcWidth = uiDefaults.getInt( "Wood.button.arc.width" );
		final int arcHeight = uiDefaults.getInt( "Wood.button.arc.height" );
		Color borderColor = Color.BLACK;
		if (( context.getComponentState() & SynthConstants.DEFAULT ) != 0)
		{
			if (( context.getComponentState() & SynthConstants.PRESSED ) != 0)
			{
				borderColor = uiDefaults.getColor( "Wood.button.border.color.default.pressed" );
			}
			else
				if (( context.getComponentState() & SynthConstants.DISABLED ) != 0)
				{
					borderColor = uiDefaults.getColor( "Wood.button.border.color.default.disabled" );
				}
				else
					if (( context.getComponentState() & SynthConstants.MOUSE_OVER ) != 0)
					{
						borderColor = uiDefaults.getColor( "Wood.button.border.color.default.mouseover" );
					}
					else
					{
						borderColor = uiDefaults.getColor( "Wood.button.border.color.default.enabled" );
					}
		}
		else
		{
			if (( context.getComponentState() & SynthConstants.PRESSED ) != 0)
			{
				borderColor = uiDefaults.getColor( "Wood.button.border.color.pressed" );
			}
			else
				if (( context.getComponentState() & SynthConstants.DISABLED ) != 0)
				{
					borderColor = uiDefaults.getColor( "Wood.button.border.color.disabled" );
				}
				else
					if (( context.getComponentState() & SynthConstants.MOUSE_OVER ) != 0)
					{
						borderColor = uiDefaults.getColor( "Wood.button.border.color.mouseover" );
					}
					else
					{
						borderColor = uiDefaults.getColor( "Wood.button.border.color.enabled" );
					}
		}
	 ...
	 * 		the height of the rectangle to paint
	 */
	public void paintToggleButtonBorder( final SynthContext context,
		final Graphics g,
											final int x,
		final int y,
		final int w,
	 ...
	 */
	public void paintArrowButtonForeground( final SynthContext context,
		final Graphics g,
											final int x,
		final int y,
		final int w,
		...
		final int direction )
	{
		final StringBuilder iconKeyBuf = new StringBuilder( "Wood.arrow." );
		...
		final ImageIcon icon = (ImageIcon) context.getStyle()
			.getIcon(
				context,
				iconKeyBuf.toString() );
		if (icon != null)
	 ...
	 * 		the height of the rectangle to paint
	 */
	public void paintRootPaneBackground( final SynthContext context,
		final Graphics g,
											final int x,
		final int y,
		final int w,
		final int h )
	{
		...
		if (icon != null)
		{
			...
			{
				final BufferedImage bufImage = new BufferedImage( imgWidth,
					imgHeight,
	...
	@Override
	public void paintProgressBarBorder( final SynthContext context,
		final Graphics g,
										final int x,
		final int y,
		final int w,
		final int h )
	{
		...
		{
			final boolean paintTextWhenIndeterminate = context.getStyle()
				.getBoolean(
					context,
					"Wood.progressbar.indeterminate.paintstring",
					false );
			final String textToPaint = progressBar.getString();
			if (paintTextWhenIndeterminate && textToPaint != null)
			{
				final Class synthProgBarUIClass = progressBar.getUI()
					.getClass();
				try
				{
					final Method paintTextMethod = synthProgBarUIClass.getDeclaredMethod(
						"paintText",
						new Class[]
							{
								SynthContext.class, Graphics.class,
								String.class
							} );
					if (paintTextMethod != null)
					{
						...
						try
						{
							final Font font = context.getStyle()
								.getFont(
									context );
							final FontMetrics metrics = Toolkit.getDefaultToolkit()
								.getFontMetrics(
									font );
							final int strLength = context.getStyle()
								.getGraphicsUtils(
									context )
								.computeStringWidth(
									context,
							...
							Insets bgInsets = (Insets) context.getStyle()
								.get(
									context,
									"Wood.progressbar.bg.insets" );
							if (bgInsets == null)
							{
						...
						paintTextMethod.invoke(
							progressBar.getUI(),
							new Object[]
								{
									context, g, textToPaint
								} );
	 ...
	 * 		the height of the rectangle to paint
	 */
	public void paintProgressBarBackground( final SynthContext context,
		final Graphics g,
											final int x,
		final int y,
		final int w,
	...
	{
		final Graphics2D g2 = (Graphics2D) g.create();
		final ImageIcon bgIcon = (ImageIcon) context.getStyle()
			.getIcon(
				context,
				"Wood.progressbar.background.image" );
		final Insets bgInsets = (Insets) context.getStyle()
			.get(
				context,
				"Wood.progressbar.bg.insets" );
		if (bgIcon != null)
		{
			WoodGraphicsUtils.drawImageWith9Grids(
				g,
				bgIcon.getImage(),
														x,
				y,
		...
		{
			final UIDefaults uiDefaults = UIManager.getDefaults();
			final Color bgColor = uiDefaults.getColor( "Wood.progressbar.background.color" );
			if (bgColor != null)
			{
				final int arcWidth = uiDefaults.getInt( "Wood.progressbar.arc.width" );
				final int arcHeight = uiDefaults.getInt( "Wood.progressbar.arc.height" );
				g2.setPaint( bgColor );
				g2.fillRoundRect(
					x,
					y,
		...
		final ImageIcon icon = (ImageIcon) context.getStyle()
			.getIcon(
				context,
				"Wood.progressbar.indication.image" );
	 ...
	 */
	public void paintProgressBarForeground( final SynthContext context,
		final Graphics g,
											final int x,
		final int y,
		final int w,
		...
		final int orientation )
	{
		...
		final UIDefaults uiDefaults = UIManager.getDefaults();
		Color lineColor = uiDefaults.getColor( "Wood.progressbar.line.color" );
		if (lineColor == null)
		{
			...
			if (JProgressBar.HORIZONTAL == orientation)
			{
				...
				if (!isDisabled)
				{
					...
					if (progressBar.isIndeterminate())
					{
						...
						final int curMid = imgWidth / 2
							+ ( progressBar.getWidth() - imgWidth ) * x
							/ midRange;
			...
			else
			{
				...
				if (!isDisabled)
				{
					...
					if (progressBar.isIndeterminate())
					{
						...
						final int curMid = imgHeight / 2
							+ ( progressBar.getHeight() - imgHeight ) * y
							/ midRange;
	 ...
	 * 		the height of the rectangle to paint
	 */
	public void paintInternalFrameBorder( final SynthContext context,
		final Graphics g,
											final int x,
		final int y,
		final int w,
	...
	{
		final UIDefaults uiDefaults = UIManager.getDefaults();
		Color mainColor = uiDefaults.getColor( "Wood.internalframe.main.color" );
		if (mainColor == null)
		{
	 ...
	 * 		the height of the rectangle to paint
	 */
	public void paintInternalFrameTitlePaneBorder( final SynthContext context,
		final Graphics g,
													final int x,
		final int y,
		final int w,
	 ...
	 * 		the height of the rectangle to paint
	 */
	public void paintInternalFrameBackground( final SynthContext context,
		final Graphics g,
												final int x,
		final int y,
		final int w,
	 ...
	 * 		the height of the rectangle to paint
	 */
	public void paintInternalFrameTitlePaneBackground( final SynthContext context,
		final Graphics g,
														final int x,
		final int y,
		final int w,
		final int h )
	{
		...
		{
			final int titleSpacing = context.getStyle()
				.getInt(
					context,
			...
			while (!( comp instanceof JInternalFrame.JDesktopIcon )
				&& comp != null)
			{
				comp = comp.getParent();
			}
	 ...
	 */
	@Override
	public void paintScrollBarTrackBackground( final SynthContext context,
		final Graphics g,
												final int x,
		final int y,
		final int w,
		final int h )
	{
		...
		final UIDefaults uiDefaults = UIManager.getDefaults();
		Color sideColor = uiDefaults.getColor( "Wood.scrollbar.track.sidecolor" );
		if (sideColor == null)
		{
			sideColor = Color.white;
		}
		Color centerColor = uiDefaults.getColor( "Wood.scrollbar.track.centercolor" );
		if (centerColor == null)
		{
	 ...
	 * 		the height of the rectangle to paint
	 */
	public void paintScrollBarTrackBorder( final SynthContext context,
		final Graphics g,
											final int x,
		final int y,
		final int w,
	...
	{
		final UIDefaults uiDefaults = UIManager.getDefaults();
		Color borderColor = uiDefaults.getColor( "Wood.scrollbar.track.bordercolor" );
		if (borderColor == null)
		{
	 ...
	 */
	@Override
	public void paintScrollBarThumbBackground( final SynthContext context,
		final Graphics g,
												final int x,
		final int y,
		final int w,
		...
		final int orientation )
	{
		final ImageIcon patternIcon = (ImageIcon) context.getStyle()
			.getIcon(
				context,
				"Wood.scrollbar.thumb.patternimage" );

		//        ShapeEffect patternFill = new ImageFillEffect( patternIcon );
		//        patternFill.draw( ( Graphics2D ) g, new Rectangle( x, y, w, h ) );
		final UIDefaults uiDefaults = UIManager.getDefaults();
		final int borderThick = uiDefaults.getInt( "Wood.scrollbar.thumb.borderthick" );
		//        Color sideColor = uiDefaults.getColor( "Wood.scrollbar.thumb.sidecolor" );
		//        if ( sideColor == null )
		//        {
		//            sideColor = Color.LIGHT_GRAY;
		//        }
		//        Color centerColor = uiDefaults.getColor( "Wood.scrollbar.thumb.centercolor" );
		//        if ( centerColor == null )
		//        {
		...
		if (isVertical)
		{
			thumbicon = (ImageIcon) context.getStyle()
				.getIcon(
					context,
					"Wood.scrollbar.thumb.image.vert" );
		}
		else
		{
			thumbicon = (ImageIcon) context.getStyle()
				.getIcon(
					context,
					"Wood.scrollbar.thumb.image.horz" );
		}
	 ...
	 * 		the orientation of the scroll bar
	 */
	public void paintScrollBarThumbBorder( final SynthContext context,
		final Graphics g,
											final int x,
		final int y,
		final int w,
		...
		final int orientation )
	{
		...
		//        final UIDefaults uiDefaults = UIManager.getDefaults();
		//        final int borderThick = uiDefaults.getInt( "Wood.scrollbar.thumb.borderthick" );
		//        Color borderColor = uiDefaults.getColor( "Wood.scrollbar.thumb.bordercolor" );
		//        if ( borderColor == null )
		//        {
		//            borderColor = Color.DARK_GRAY;
		//        }
	 ...
	 * 		the height of the rectangle to paint
	 */
	public void paintScrollBarBackground( final SynthContext context,
		final Graphics g,
											final int x,
		final int y,
		final int w,
	...
	{
		final UIDefaults uiDefaults = UIManager.getDefaults();
		Color bgColor = uiDefaults.getColor( "Wood.scrollbar.bgcolor" );
		if (bgColor == null)
		{
			bgColor = Color.WHITE;
		}
	 ...
	 * 		the height of the rectangle to paint
	 */
	public void paintScrollBarBorder( final SynthContext context,
		final Graphics g,
										final int x,
		final int y,
		final int w,
	...
	{
		final UIDefaults uiDefaults = UIManager.getDefaults();
		Color borderColor = uiDefaults.getColor( "Wood.scrollbar.bordercolor" );
		if (borderColor == null)
		{
			borderColor = Color.DARK_GRAY;
		}
	 ...
	 * 		the height of the rectangle to paint
	 */
	public void paintSplitPaneDividerBackground( final SynthContext context,
		final Graphics g,
													final int x,
		final int y,
		final int w,
	...
	{
		final UIDefaults uiDefaults = UIManager.getDefaults();
		Color mainColor = uiDefaults.getColor( "Wood.splitpane.divider.maincolor" );
		if (mainColor == null)
		{
	 ...
	 */
	public void paintSplitPaneDragDivider( final SynthContext context,
		final Graphics g,
											final int x,
		final int y,
		final int w,
	 ...
	 * 		the height of the rectangle to paint
	 */
	public void paintTableHeaderBorder( final SynthContext context,
		final Graphics g,
										final int x,
		final int y,
		final int w,
		...
		synchronized (MANAGED_OBJECT_MAP)
		{
			if (!MANAGED_OBJECT_MAP.containsKey( context.getComponent() ))
			{
				if (context.getComponent() instanceof JComboBox)
				{
					...
					{
						if (l instanceof WoodComboBoxPopupMenuListener)
						{
							alreadyFixed = true;
							break;
						}
					...
					if (!alreadyFixed)
					{
						comboBox.addPopupMenuListener( new WoodComboBoxPopupMenuListener() );
						MANAGED_OBJECT_MAP.put(
							comboBox,
							comboBox.getClass()
								.getName() );
					}
	 ...
	 * (do not recommend from Wood Look And Feel Template V1.20)
	 * 
	 * @param context
	 * 		the SynthContext object for painting
	 * @param g
	 ...
	 * 		the height of the rectangle to paint
	 */
	public void paintComboBoxBackground( final SynthContext context,
		final Graphics g,
											final int x,
		final int y,
		final int w,
		final int h )
	{
		...
		if (( context.getComponentState() & SynthConstants.DISABLED ) != 0)
		{
			icon = (ImageIcon) context.getStyle()
				.getIcon(
					context,
					"Wood.combobox.bg.image.disabled" );
		}
		else
		{
			icon = (ImageIcon) context.getStyle()
				.getIcon(
					context,
					"Wood.combobox.bg.image.enabled" );
		}
		if (icon != null)
		{
			final Image image = icon.getImage();
	 ...
	 * 		the height of the rectangle to paint
	 */
	public void paintMenuBackground( final SynthContext context,
		final Graphics g,
										final int x,
		final int y,
		final int w,
	...
	{
		final UIDefaults uiDefaults = UIManager.getDefaults();
		Color normalColor = uiDefaults.getColor( "Wood.menu.bg.color.normal" );
		if (normalColor == null)
		{
			normalColor = Color.LIGHT_GRAY;
		}
		Color selectedColor = uiDefaults.getColor( "Wood.menu.bg.color.selected" );
	 ...
	 */
	public void paintSliderThumbBackground( final SynthContext context,
		final Graphics g,
											final int x,
		final int y,
		final int w,
		...
		final int orientation )
	{
		...
		if (( context.getComponentState() & SynthConstants.DISABLED ) != 0)
		{
			thumbIcon = (ImageIcon) context.getStyle()
				.getIcon(
					context,
					"Wood.slider.thumb.image.disabled" );
		}
		else
			if (( context.getComponentState() & SynthConstants.MOUSE_OVER ) != 0)
			{
				thumbIcon = (ImageIcon) context.getStyle()
					.getIcon(
						context,
						"Wood.slider.thumb.image.mouseover" );
			}
			else
			{
				thumbIcon = (ImageIcon) context.getStyle()
					.getIcon(
						context,
						"Wood.slider.thumb.image.normal" );
			}
		if (thumbIcon != null)
		{
	 ...
	 * 		the height of the rectangle to paint
	 */
	public void paintToolBarBackground( final SynthContext context,
		final Graphics g,
										final int x,
		final int y,
		final int w,
	...
	{
		final UIDefaults uiDefaults = UIManager.getDefaults();
		Color color1 = uiDefaults.getColor( "Wood.toolbar.bg.color1" );
		if (color1 == null)
		{
			color1 = Color.LIGHT_GRAY;
		}
		Color color2 = uiDefaults.getColor( "Wood.toolbar.bg.color2" );
		if (color2 == null)
		{
			color2 = Color.WHITE;
		}
		Color color3 = uiDefaults.getColor( "Wood.toolbar.bg.color3" );
		if (color3 == null)
		{
		...
		int h )
	{
		...
		final BufferedImage bgImage = (BufferedImage) ( panel.getClientProperty( WoodPopup.POPUP_BACKGROUND_IMAGE ) );
		if (bgImage != null)
		{
			g.drawImage(
				bgImage,
