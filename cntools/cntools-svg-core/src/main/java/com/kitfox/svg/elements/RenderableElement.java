...
abstract public class RenderableElement extends TransformableElement
{
	...
	private AffineTransform cachedXform = null;
	...
	protected ElementReferenceAttribute clipPathElement = new ElementReferenceAttribute( "clip-path",
		this );

	//int vectorEffect;
	IntAttribute vectorEffect = new IntAttribute( "vector-effect" )
	{

		@Override
		public void setStringValue( String value )
		{
			setValue( value.equals( "non-scaling-stroke" ) ?
	...
	private void init()
	{
		addPresentationAttributes( vectorEffect );
	}
	...
	public ClipPath getClipElement()
	{
		if (clipPathElement.isSet())
		{
			return (ClipPath) clipPathElement.getElement();
		}

		ElementReferenceAttribute clipPathAttribute = new ElementReferenceAttribute( "clip-path",
			this );
		try
		{
			if (getStyleFromParent( clipPathAttribute ))
			{
				return (ClipPath) clipPathAttribute.getElement();
			}
		}
		catch (SVGException ex)
		{
			Logger.getLogger(
				RenderableElement.class.getName() )
				.log(
					Level.SEVERE,
					null,
					ex );
	...
	abstract void pick( Point2D point, boolean boundingBox, List retVec )
		throws SVGException;

	abstract void pick( Rectangle2D pickArea,
		AffineTransform ltw,
		boolean boundingBox,
		List retVec ) throws
			SVGException;
	 ...
	 * SVGElement parent) { super.loaderStartElement(helper, attrs, parent); }
	 */
	...
	protected void beginLayer( Graphics2D g ) throws SVGException
	{
		...
		//Get clipping path
		ClipPath clipEl = getClipElement();
		Shape clipPath = null;
		int clipPathUnits = ClipPath.CP_USER_SPACE_ON_USE;
		if (clipEl != null)
		{
			clipPath = clipEl.getClipPathShape();
			clipPathUnits = clipEl.getClipPathUnits();
		}
		...
		if (clipPath != null)
		{
			if (clipPathUnits == ClipPath.CP_OBJECT_BOUNDING_BOX
				&& ( this instanceof ShapeElement ))
			{
			...
			if (cachedClip != null)
			{
				clipArea.intersect( new Area( cachedClip ) );
			}
			g.setClip( clipArea );
		}
	}
