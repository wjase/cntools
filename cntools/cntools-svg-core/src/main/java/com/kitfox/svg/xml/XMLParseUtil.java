...
import com.kitfox.svg.xml.attributes.NamedAttribute;
...
{
	static final Matcher fpMatch = Pattern.compile(
		"([-+]?((\\d*\\.\\d+)|(\\d+))([eE][+-]?\\d+)?)(\\%|in|cm|mm|pt|pc|px|em|ex)?" )
		.matcher(
			"" );
	static final Matcher intMatch = Pattern.compile(
		"[-+]?\\d+" )
		.matcher(
			"" );
	...
	public static String[] parseStringList( String list )
	{
		...
		final Matcher matchWs = Pattern.compile(
			"[^\\s]+" )
			.matcher(
				"" );
		matchWs.reset( list );
		...
		URL docRoot )
	{
		...
		LinkedList elementCache = getElementLinkedList(
			classType,
			root,
			name,
			docRoot );

		Object[] retArr = (Object[]) Array.newInstance(
			classType,
			elementCache.size() );
	...
	public static HashMap parseStyle( String styleString, HashMap map )
	{
		...
		final Matcher matcherContent = Pattern.compile(
			"\\s*([^\\s](.*[^\\s])?)\\s*" )
			.matcher(
				"" );
		...
		for (int i = 0; i < styles.length; i++)
		{
			...
			map.put(
				vals[ 0 ],
				new StyleAttribute( vals[ 0 ], vals[ 1 ] ) );
		}
		...
		Map< String, IStringAttribute > map )
	{
		...
		for (int i = 0; i < styles.length; ++i)
		{
			...
			map.put(
				vals[ 0 ],
				new StringAttribute( vals[ 0 ], vals[ 1 ] ) );
		}
