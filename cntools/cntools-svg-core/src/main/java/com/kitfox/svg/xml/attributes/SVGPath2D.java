...
/**
 * The {@code OpenPath2D} class provides a simple, yet flexible
 ...
 * <p>
 * Use {@link OpenPath2D.Float} when dealing with data that can be represented
 * and used with floating point precision. Use {@link OpenPath2D.Double}
 * for data that requires the accuracy or range of double precision.
 * <p>
 * {@code OpenPath2D} provides exactly those facilities required for
 ...
 * two useful views of a geometric shape where {@code OpenPath2D}
 ...
 * @since 1.6
 */
public abstract class SVGPath2D implements Shape, Cloneable, IMutablePath
{
	...
	// additional bitmasks indicating SVG path info which isn't contained 
	// in Java2D paths
	private static final byte RELATIVE = (byte) 32;

	private static final byte SMOOTH = (byte) 64;

	public static final String[] segNames =
	{
	...
	/**
	 * Constructs a new empty {@code OpenPath2D} object.
	...
	/**
	 * Constructs a new {@code OpenPath2D} object from the given
	 ...
	 * private protected
	 */
	SVGPath2D( int rule, int initialTypes )
	{
		setWindingRule( rule );
		this.pointTypes = new byte[ initialTypes ];
		this.offsetFlags = new boolean[ initialTypes ];
	}
	 ...
	 * @return
	 */
	abstract Point2D getPreviousCtrl();

	abstract void setPreviousCtrl( double x, double y );

	abstract void clearCtrl();

	public void parseSVG( String value )
	{
		parseSVGPath( value );
	}
	...
	public String asSVGString()
	{
		if (svgString == null)
		{
			...
			Iterator iter = (Iterator) getPathIterator( null );
			...
			while (!iter.isDone())
			{
				...
				boolean isRelative = iter.currentIsRelative();
				boolean isSmooth = iter.currentIsSmooth();
				switch (type & 0xf)
				{
					case SEG_LINETO:
						prevCmd = appendSvgLineTo(
							sbNew,
							prevCmd,
							isRelative,
							coords[ 0 ],
						...
						break;
					case SEG_CUBICTO:
						prevCmd = appendSvgCubicTo(
							sbNew,
							prevCmd,
							isRelative,
							isSmooth,
							iter.getPreviousPoint(),
													coords );
						break;
					case SEG_MOVETO:
						prevCmd = appendSvgMoveTo(
							sbNew,
							prevCmd,
							isRelative,
							iter.getPreviousPoint(),
	...
	static char appendSvgLineTo( StringBuilder sb,
		char prevCmd,
		boolean isRelative,
		double x,
		double y,
		Point2D prev )
	{
		...
		if (java.lang.Double.compare(
			...
			y ) == 0)
		{
			typeStr = isRelative ? 'h' : 'H';
			showy = false;
		}
		boolean writeCmd = true;
		if (prevCmd == typeStr || ( prevCmd == 'M' || prevCmd == 'L' )
			&& typeStr == 'L')
		{
			writeCmd = false;
		}
	...
	private static final Pattern matchPathCmdPattern = Pattern.compile(
			"([MmLlHhVvAaQqTtCcSsZz])|([-+]?((\\d*\\.\\d+)|(\\d+))([eE][-+]?\\d+)?)" );

	public void parseSVGPath( String list )
	{
		Matcher matchPathCmd = matchPathCmdPattern.matcher( list );
		...
		while (tokens.size() != 0)
		{
			try
			{
				...
				switch (curCmd)
				{
					...
					case 'q':
						...
						break;
					case 'T':
						//TODO: how is this different from Q??
						smoothQuadTo(
							nextFloat( tokens ),
							nextFloat( tokens ) );
			...
			//          cmd.appendPath( this, null );
		}

	}

	private char appendSvgMoveTo( StringBuilder sbNew,
		char prevCmd,
		boolean isRelative,
									...
									double y )
	{
		...
		if (isRelative)
		{
			offsetx = previousPoint.getX();
			offsety = previousPoint.getY();
		}
		cmdChar = isRelative ? 'm' : 'M';
		sbNew.append( cmdChar );
		append(
			sbNew,
			x - offsetx );
		append(
	...
	private char appendSvgCubicTo( StringBuilder sbNew,
		char prevCmd,
		boolean isRelative,
		boolean isSmooth,
									Point2D previousPoint,
									float[] coords )
	{
		...
		if (isSmooth)
		{
			cmdChar = isRelative ? 's' : 'S';
			sbNew.append( cmdChar );

		}
		else
		{
			cmdChar = isRelative ? 'c' : 'C';
			sbNew.append( cmdChar );
			append(
				sbNew,
				coords[ 0 ] - offsetx );
	...
	private char appendSvgQuadTo( StringBuilder sbNew,
		char prevCmd,
		boolean isRelative,
		boolean isSmooth,
									Point2D previousPoint,
									float[] coords )
	{
		...
		if (isSmooth)
		{
			cmdChar = isRelative ? 't' : 'T';
			sbNew.append( cmdChar );
		}
		else
		{
			cmdChar = isRelative ? 'q' : 'Q';
			sbNew.append( cmdChar );
			append(
				sbNew,
				coords[ 0 ] );
	 ...
	 * @since 1.6
	 */
	public static class Float extends SVGPath2D implements Serializable
	{
		...
		Point2D prevControl = new Point2D.Double( 0, 0 );

		boolean prevControlSet = false;

		/**
		 * Constructs a new empty single precision {@code OpenPath2D} object
		...
		/**
		 * Constructs a new single precision {@code OpenPath2D} object
		...
		{
			if (s instanceof SVGPath2D)
			{
				SVGPath2D p2d = (SVGPath2D) s;
				setWindingRule( p2d.windingRule );
				this.numTypes = p2d.numTypes;
				this.pointTypes = Arrays.copyOf(
					p2d.pointTypes,
													p2d.pointTypes.length );
				this.offsetFlags = new boolean[ p2d.pointTypes.length ];
			...
			else
			{
				...
				this.offsetFlags = new boolean[ INIT_SIZE ];
		...
		void needRoom( boolean needMove, int newCoords )
		{
			...
			if (numTypes >= size)
			{
				...
				pointTypes = Arrays.copyOf(
					pointTypes,
					size + grow );
				offsetFlags = Arrays.copyOf(
					offsetFlags,
					size + grow );
			}
		...
		public synchronized void moveTo( double x, double y )
		{
			...
			else
			{
				needRoom(
					false,
					2 );
				offsetFlags[ numTypes ] = false;
				pointTypes[ numTypes++ ] = SEG_MOVETO;
				floatCoords[ numCoords++ ] = (float) x;
				floatCoords[ numCoords++ ] = (float) y;
			}
			prevPoint.setLocation(
				x,
				y );

		}

		public synchronized void moveToRelative( double x, double y )
		{
			moveTo(
				prevPoint.getX() + x,
				prevPoint.getY() + y );
			offsetFlags[ numTypes - 1 ] = true;
			pointTypes[ numTypes - 1 ] |= RELATIVE;
		 ...
		 * base {@code OpenPath2D} class.
		 *
		 * @param x the specified X coordinate
		 * @param y the specified Y coordinate
		 * @see OpenPath2D#moveTo
		 * @since 1.6
		 */
		...
		public synchronized void moveToRelative( float x, float y )
		{
			moveTo(
				prevPoint.getX() + x,
				prevPoint.getY() + y );
		...
		public synchronized void lineToRelative( double x, double y )
		{
			lineTo(
				x + prevPoint.getX(),
				y + prevPoint.getY() );
		 ...
		 * @since 1.6
		 */
		public synchronized void quadTo( double x1, double y1,
											double x2, double y2 )
		{
			needRoom(
				true,
		...
		public synchronized void quadToRelative( double x1, double y1,
													double x2, double y2 )
		{
			double x0 = prevPoint.getX();
			double y0 = prevPoint.getY();
			quadTo(
				x0 + x1,
				y0 + y1,
				x0 + x2,
		...
		public synchronized void quadToRelative( float x1, float y1,
													float x2, float y2 )
		{
			double x0 = prevPoint.getX();
			double y0 = prevPoint.getY();
		 ...
		 * @since 1.6
		 */
		public synchronized void curveTo( double ctrl1X, double ctrl1Y,
											...
											double toX, double toY )
		{
			...
			floatCoords[ numCoords++ ] = (float) ctrl1X;
			floatCoords[ numCoords++ ] = (float) ctrl1Y;
			floatCoords[ numCoords++ ] = (float) ctrl2X;
			floatCoords[ numCoords++ ] = (float) ctrl2Y;
			floatCoords[ numCoords++ ] = (float) toX;
			floatCoords[ numCoords++ ] = (float) toY;
			prevPoint.setLocation(
				toX,
				toY );
			setPreviousCtrl(
		...
		public synchronized void curveToRelative( double x1, double y1,
													double x2, double y2,
													double x3, double y3 )
		{
			double x0 = prevPoint.getX();
		 ...
		 * @since 1.6
		 */
		public synchronized void smoothCurveTo( double ctrl2X, double ctrl2Y,
												double toX, double toY )
		{
			Point2D ctrlOld = getPreviousCtrl();
			Point2D prev = getPrevious();

			double ctrlX = prev.getX() * 2f - ctrlOld.getX();
			double ctrlY = prev.getY() * 2f - ctrlOld.getY();
		...
		public synchronized void smoothCurveToRelative( double ctrl2X,
			double ctrl2Y,
														double toX,
			double toY )
		{
			...
			double newX = toX + prev.getX();
			double newY = toX + prev.getY();

			double newCtrl2X = ctrl2X + prev.getX();
			double newCtrl2Y = ctrl2Y + prev.getY();
		 ...
		 * which means that the {@code OpenPath2D} class does not
		 * guarantee that modifications to the geometry of this
		 * {@code OpenPath2D} object do not affect any iterations of
		...
		public PathIterator getPathIterator( AffineTransform at )
		{
			if (at == null)
			{
				return new SVGPath2D.Float.CopyIterator( this );
			}
			else
			{
				return new SVGPath2D.Float.TxIterator( this, at );
			}
		}
		...
		public Object clone()
		{
			return new SVGPath2D.Float( this );
		}
		 ...
		 * <td>0x30</td>
		 * <td></td>
		 * <td>A hint that the original {@code OpenPath2D} object stored
		...
		@Override
		void setPreviousCtrl( double x, double y )
		{
			prevControlSet = true;
			prevControl.setLocation(
				x,
				y );
		}
		...
		void clearCtrl()
		{
			prevControlSet = false;
		}

		@Override
		public void smoothQuadTo( double toX, double toY )
		{
			Point2D ctrlOld = getPreviousCtrl();
			Point2D prev = getPrevious();
		...
		@Override
		public void smoothQuadToRelative( double toX, double toY )
		{
			Point2D ctrlOld = getPreviousCtrl();
			Point2D prev = getPrevious();
		...
		static class CopyIterator extends SVGPath2D.Iterator
		{

			float floatCoords[];

			CopyIterator( SVGPath2D.Float p2df )
			{
				super( p2df );
			...
			@Override
			protected double getCoord( int index )
			{
				return floatCoords[ index ];
			}

			public int currentSegment( float[] coords )
			{
				int type = path.pointTypes[ typeIdx ] & 0xf;
		...
		static class TxIterator extends SVGPath2D.Iterator
		{
			...
			TxIterator( SVGPath2D.Float p2df, AffineTransform at )
			{
				super( p2df );
				this.floatCoords = p2df.floatCoords;
				this.affine = at;
	 ...
	 * @since 1.6
	 */
	public static class Double extends SVGPath2D implements Serializable
	{
		...
		private boolean prevControlSet;
		...
		/**
		 * Constructs a new empty double precision {@code OpenPath2D} object
		 * with a default winding rule of {@link #WIND_NON_ZERO}.
		 *
		 * @since 1.6
		 */
		...
		/**
		 * Constructs a new double precision {@code OpenPath2D} object
		 * from an arbitrary {@link Shape} object.
		 * All of the initial geometry and the winding rule for this path are
		 * taken from the specified {@code Shape} object.
		 *
		...
		public Double( Shape s, AffineTransform at )
		{
			if (s instanceof SVGPath2D)
			{
				...
				this.pointTypes = Arrays.copyOf(
					p2d.pointTypes,
													p2d.pointTypes.length );
				this.offsetFlags = Arrays.copyOf(
					p2d.offsetFlags,
					p2d.offsetFlags.length );
				this.numCoords = p2d.numCoords;
		...
		public void moveToRelative( double x, double y )
		{
			moveTo(
				x + prevPoint.getX(),
				prevPoint.getY() + y );
			this.offsetFlags[ numTypes - 1 ] = true;
			pointTypes[ numTypes - 1 ] |= RELATIVE;
		...
		public PathIterator getPathIterator( AffineTransform at )
		{
			if (at == null)
			{
				return new SVGPath2D.Double.CopyIterator( this );
			}
			else
			{
				return new SVGPath2D.Double.TxIterator( this, at );
			}
		}
		...
		public Object clone()
		{
			// Note: It would be nice to have this return OpenPath2D
			...
			return new SVGPath2D.Double( this );
		}
		...
		static class CopyIterator extends SVGPath2D.Iterator
		{
			...
			CopyIterator( SVGPath2D.Double p2dd )
			{
				super( p2dd );
		...
		static class TxIterator extends SVGPath2D.Iterator
		{
			...
			TxIterator( SVGPath2D.Double p2dd, AffineTransform at )
			{
				super( p2dd );
				this.doubleCoords = p2dd.doubleCoords;
				this.affine = at;
	...
	public void arcToRelative( float rx, float ry,
								float angle,
								boolean largeArcFlag,
								boolean sweepFlag,
								float x, float y )
	{
		double x0 = getPrevious().getX();
		double y0 = getPrevious().getY();
		arcTo(
			rx,
			ry,
			angle,
	 ...
	 * for the transformed geometry than this {@code OpenPath2D} currently
	 ...
	 * {@link OpenPath2D.Float#OpenPath2D.Float(Shape, AffineTransform) OpenPath2D.Float}
	 * and
	 * {@link OpenPath2D.Double#OpenPath2D.Double(Shape, AffineTransform) OpenPath2D.Double}
	...
	public static boolean contains( PathIterator pi, double x, double y )
	{
		if (x * 0.0 + y * 0.0 == 0.0)
		{
			...
			int cross = Curve.pointCrossingsForPath(
				pi,
				x,
				y );
			return ( ( cross & mask ) != 0 );
									...
									double x, double y, double w, double h )
	{
		...
		int crossings = Curve.rectCrossingsForPath(
			pi,
			x,
			y,
			x + w,
	 ...
	 * <p>
	 * The iterator for this class is not multi-threaded safe,
	 * which means that this {@code OpenPath2D} class does not
	 * guarantee that modifications to the geometry of this
	 * {@code OpenPath2D} object do not affect any iterations of
	 * that geometry that are already in process.
	 *
			...
			throws java.io.IOException
	{
		...
		if (isdbl)
		{
			dCoords = ( (SVGPath2D.Double) this ).doubleCoords;
			fCoords = null;
		}
		else
		{
			fCoords = ( (SVGPath2D.Float) this ).floatCoords;
			dCoords = null;
		}
			...
			throws java.lang.ClassNotFoundException, java.io.IOException
	{
		...
		if (storedbl)
		{
			( (SVGPath2D.Double) this ).doubleCoords = new double[ nC ];
		}
		else
		{
			( (SVGPath2D.Float) this ).floatCoords = new float[ nC ];
		}
	...
	static abstract class Iterator implements PathIterator
	{
		...
		protected SVGPath2D path;

		protected Point2D previousPoint = new Point2D.Double();
		...
		// If this point was initially converted from offsets
		// relative tot he previous point
		public boolean currentIsRelative()
		{
			return ( path.pointTypes[ typeIdx ] & RELATIVE ) > 0;
		}

		public boolean currentIsSmooth()
		{
			return ( path.pointTypes[ typeIdx ] & SMOOTH ) > 0;
		}
		...
		abstract protected double getCoord( int index );
