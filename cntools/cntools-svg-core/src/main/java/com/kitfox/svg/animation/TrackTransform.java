...
public class TrackTransform extends TrackBase
{
	...
	public boolean getValue( IStringAttribute attrib, double curTime )
		throws SVGException
	{
	...
	public AffineTransform getValue( AffineTransform retVal, double curTime )
		throws SVGException
	{
		//Init transform with default state
		IStringAttribute attr = null;
		switch (attribType)
		{
		...
		if (attr != null)
		{
			...
			{
				retVal.setTransform( ( (TransformAttribute) attr ).getXform() );
			}
			else
			{
		...
		for (Iterator it = animEvents.iterator(); it.hasNext();)
		{
			...
			switch (ele.getAdditiveType())
			{
				case AnimationElement.AD_SUM:
					retVal.concatenate( ele.eval(
						xform,
						state.interp ) );
					break;
				case AnimationElement.AD_REPLACE:
					retVal.setTransform( ele.eval(
						xform,
						state.interp ) );
					break;
			}
