 ...
 */
abstract public class SVGElement implements Serializable, AttributeListener
{
	...
	private StringAttribute id = new StringAttribute( "id" );

	private BooleanAttribute visibility = new BooleanAttribute( "visibility",
		VisibilityAdapter.get() );

	/**
	 * CSS class. Used for applying style sheet information.
	 */
	protected StringAttribute cssClass = new StringAttribute( "class" );

	/**
	 * Styles defined for this element via the <b>style</b> attribute.
	 *
	 */
	protected CSSStyleAttribute CSSStyles = new CSSStyleAttribute( "style" );

	public CSSStyleAttribute getCSS()
	{
		return CSSStyles;
	}

	protected URIAttribute xmlBase = new URIAttribute( "xml:base" );
	 ...
	 */
	protected final HashMap< String, IStringAttribute > presAttribs = new HashMap< String, IStringAttribute >();

	public Collection< IStringAttribute > getAttributes()
	{
		return presAttribs.values();
	}
	...
	public SVGElement( String id, String cssClass, SVGElement parent )
	{
		...
		this.cssClass.setStringValue( cssClass );
		this.parent = parent;
		this.visibility.setDefault( true );
		addPresentationAttributes(
			CSSStyles,
			this.id,
			this.cssClass,
			this.xmlBase );
		bindStyleAttributes( visibility );
	...
	public boolean isVisible()
	{
		if (visibility.isSet())
		{
			return visibility.getBooleanValue();

		}
		ParentAttribute parentVisible = getStyleFromParent( "visibility" );

		if (parentVisible != null)
		{
			BooleanAttribute parentVisibility = (BooleanAttribute) parentVisible.getParentValue();
			if (parentVisibility.isSet())
			{
				return parentVisibility.getBooleanValue();
			}
		}
		return true;
	}
		...
		SVGElement parent ) throws SAXException
	{
		...
		for (Entry< String, IStringAttribute > eachAttribute : CSSStyles.entrySet())
		{
			if (eachAttribute.getValue()
				.isSet())
			{
				initStyleAtttributeValue(
					eachAttribute.getKey(),
					eachAttribute.getValue()
						.getStringValue() );
			}
		}
	...
	/**
	 * Binds updates to the attributeUpdated method on this object
	 */
	protected final void bindStyleAttributes( IStringAttribute... attr )
	{
		for (IStringAttribute eachAttribute : attr)
		{
			if (eachAttribute instanceof MutableAttribute)
			{
				( (MutableAttribute) eachAttribute ).setAttributeListener( this );
			}
			CSSStyles.put( eachAttribute );
		}
	}

	public boolean setStyleAttribute( String name,
		String value,
		IStringAttribute... attr )
	{
		for (IStringAttribute eachAttribute : attr)
		{
					...
					name ))
			{
				...
				{
					eachAttribute.setStringValue( value );
				}
				// add the attribute if it hasn't already been
				CSSStyles.put( eachAttribute );
				return true;
			}
		}
	...
	protected boolean addPresentationAttribute( String name,
		String value,
		IStringAttribute... attr )
	{
		for (IStringAttribute eachAttribute : attr)
		{
			if (eachAttribute.getName()
				.equals(
					name ))
			{
				addPresentationAttribute( eachAttribute );
	...
	protected final void addPresentationAttributes( IStringAttribute... attr )
	{
		for (IStringAttribute eachAtt : attr)
		{
			addPresentationAttribute( eachAtt );
		}
	}

	public final void addPresentationAttribute( IStringAttribute attr )
	{
		presAttribs.put(
			attr.getName(),
			attr );
		if (attr instanceof MutableAttribute)
		{
			( (MutableAttribute) attr ).setAttributeListener( this );
		}

	}

	// init values parsed from the inline style
	...
	public boolean initAtttributeValue( String name, String value )
	{
		switch (name.charAt( 0 ))
		{
			case 'c':
				...
				//                {
				//                    cssClass.setValue( ( value == null || value.equals( "" ) ) ? null : value);
				//                    presAttribs.put( cssClass.getName(), cssClass );
				//                }
			case 'i':
				if (addPresentationAttribute(
					name,
			...
			case 's':
				...
				//                {
				//                    XMLParseUtil.parseStyle( value, inlineStyles );
				//                }
		...
		// not a common attribute  so just add a generic version
		StringAttribute saGeneric = new StringAttribute( name );
		...
		throws SVGElementException
	{
			...
			attribType ))
		{
			throw new SVGElementException( this, "Attribute " + name + "("
				+ AnimationElement.animationElementToString(
					attribType ) + ") already exists" );
		}

		//Alter layout for id attribute
		if ("id".equals( name ) && diagram != null)
		{
			diagram.removeElement( this.id.getStringValue() );
			this.id.setStringValue( name );
			diagram.setElement(
		...
		throws SVGElementException
	{
		...
		{
			case AnimationElement.AT_CSS:
				return CSSStyles.containsKey( name );
			case AnimationElement.AT_XML:
	...
	private Set< ElementHierachyListener > hierachyListeners = new HashSet< ElementHierachyListener >();

	public void addHierachyListener( ElementHierachyListener listener )
	{
		hierachyListeners.add( listener );
	}

	public void removeHierachyListener( ElementHierachyListener listener )
	{
		hierachyListeners.remove( listener );
	}

	public void fireAdded()
	{
		for (ElementHierachyListener eachListener : hierachyListeners)
		{
			eachListener.addedToParent( parent );
		}
		hierachyListeners.clear();
	}
	...
	protected void build() throws SVGException
	{
		...
		//            if ( !newId.equals( id.getStringValue() ) )
		//            {
		//                diagram.removeElement( id.getStringValue() );
		//                id.setValue( newId );
		//                diagram.setElement( this.id.getStringValue(), this );
		//            }
		//        }
	...
	{
		URI val = xmlBase.getURIValue();
		return val != null ? val :
				( parent != null ? parent.getXMLBase() :
					( diagram != null ? diagram.getXMLBase() : null ) );
	}
	 ...
	 * one found.
	 */
	public boolean getStyle( IStringAttribute attrib ) throws SVGException
	{
		return getStyle(
			attrib,
			true );
	...
	public void setAttribute( String name, int attribType, String value )
		throws SVGElementException
	{
		IStringAttribute styAttr;

		switch (attribType)
		{
			case AnimationElement.AT_CSS:
			{
				styAttr = (IStringAttribute) CSSStyles.get( name );
				break;
			}
			case AnimationElement.AT_XML:
			{
				styAttr = (IStringAttribute) presAttribs.get( name );
				break;
			}
			case AnimationElement.AT_AUTO:
			{
	 ...
	 * inline styles first, then internal and external style sheets, and
	 ...
	 * one found.
	 */
	public boolean getStyle( IStringAttribute attrib, boolean recursive )
		throws SVGException
	{
		String styName = attrib.getName();

		//Check for local inline styles
		IStringAttribute styAttr = (IStringAttribute) CSSStyles.get( styName );

		attrib.setStringValue( styAttr == null ? "" : styAttr.getStringValue() );

		//Evalutate coresponding track, if one exists
		TrackBase track = trackManager.getTrack(
			styName,
		...
		if (styAttr != null)//&& !styAttr.getStringValue().equalsIgnoreCase("none") )
		{
			return true;
		}
		...
		IStringAttribute presAttr = (IStringAttribute) presAttribs.get( styName );

		attrib.setStringValue( presAttr == null ? ""
			: presAttr.getStringValue() );
		...
		if (recursive)
		{
			return getStyleFromParent( attrib );
		}
	...
	protected boolean getStyleFromParent( IStringAttribute toGet )
		throws SVGException
	{
	...
	protected ParentAttribute getStyleFromParent( String name )
	{
		SVGElement parentContext = getParentContext();
		if (parentContext != null)
		{
			IStringAttribute parentValue = parentContext.getStyle( name );
			if (parentValue != null)
			{
				return new ParentAttribute( this, parentValue );
			}

		}
		if (parent != null)
		{
			IStringAttribute parentValue = parent.getStyle( name );
			if (parentValue != null)
			{
				return new ParentAttribute( this, parentValue );
			}
	...
	public IStringAttribute getStyle( String name )
	{
		IStringAttribute toGet = getStyleAbsolute( name );
		if (toGet == null)
		{
			// wrap in parent attribute to redirect updates to this element
			toGet = getStyleFromParent( name );
		}
		return toGet;
	}
	 ...
	 */
	public IStringAttribute getStyleAbsolute( String styName )
	{
		//Check for local inline styles
		return (IStringAttribute) CSSStyles.get( styName );
	}
	 ...
	 */
	public boolean getPres( IStringAttribute attrib ) throws SVGException
	{
		String presName = attrib.getName();

		//Make sure we have a coresponding presentation attribute
		IStringAttribute presAttr = (IStringAttribute) presAttribs.get( presName );
	 ...
	 */
	public IStringAttribute getPresAbsolute( String styName )
	{
		//Check for local inline styles
		return (IStringAttribute) presAttribs.get( styName );
	}
	...
	//        return retArr;
	//    }
	static protected SVGPath2D buildPath( String text, int windingRule )
	{
		SVGPath2D svgp = new SVGPath2D.Double();
		svgp.parseSVG( text );
		return svgp;
	...
	{
		//System.out.printf( "%s attributeUpdated %s\n", source.getClass().getSimpleName(), source.asSVGString() );
	}

	public void setStyleAttribute( IStringAttribute toAdd )
	{
		CSSStyles.put( toAdd );
		bindStyleAttributes( toAdd );
	}
