...
public class SVGPlayer extends javax.swing.JFrame
{
	...
	final JFileChooser fileChooser;
	{
		...
		try
		{
			...
			fc.setFileFilter(
				...
				{
					final Matcher matchLevelFile = Pattern.compile(
						".*\\.svg[z]?" )
						.matcher(
							"" );
	...
	public SVGPlayer()
	{
		...
		svgDisplayPanel.addMouseListener( new MouseAdapter()
		{
			public void mouseClicked( MouseEvent evt )
			{
				...
				System.out.println( "Picking at cursor (" + evt.getX() + ", "
					+ evt.getY() + ")" );
				try
				{
					List paths = diagram.pick(
	...
	{//GEN-HEADEREND:event_cm_loadUrlActionPerformed
		String urlStrn = JOptionPane.showInputDialog(
			this,
			"Enter URL of SVG file" );
		if (urlStrn == null)
			return;

		try
		{
			URL url = new URL( URLEncoder.encode(
				urlStrn,
				"UTF-8" ) );
			loadURL( url );
		}
	...
	{//GEN-HEADEREND:event_cm_aboutActionPerformed
		VersionDialog dia = new VersionDialog( this,
			true,
	...
	{//GEN-FIRST:event_cm_800x600ActionPerformed
		setSize(
			800,
			600 );
