...
public class Animate extends AnimateBase implements AnimateColorIface
{
	...
	protected Path2DAttribute fromPath = new Path2DAttribute( "from" );
	protected Path2DAttribute toPath = new Path2DAttribute( "to" );
		...
		SVGElement parent ) throws SAXException
	{
		...
		if (strn != null)
		{
			...
			else
			{
				...
				{
					//Try path
					fromPath.setStringValue( strn );// = this.buildPath(strn, GeneralPath.WIND_EVEN_ODD);
					dataType = DT_PATH;
				}
				else
					dataType = DT_COLOR;
		...
		{
			if (strn != null)
				byValue = XMLParseUtil.parseDouble( strn );
		}
		catch (Exception e)
		{
		}
		...
		{
			if (strn != null)
				valuesValue = XMLParseUtil.parseDoubleList( strn );
		}
		catch (Exception e)
		{
		}
	...
	public double eval( double interp )
	{
		...
		if (valuesExists)
		{
			...
			if (i0 < 0)
				return valuesValue[ 0 ];
			if (i1 >= valuesValue.length)
				return valuesValue[ valuesValue.length - 1 ];
	...
	public Color evalColor( double interp )
	{
		if (fromColor == null && toColor != null)
		{
			...
			if (fromColor != null && toColor != null)
			{
				...
				toColor.getColorComponents( toCol );
				return new Color( fromCol[ 0 ] * nInterp + toCol[ 0 ]
					* (float) interp,
	...
	public GeneralPath evalPath( double interp )
	{
		...
		{
			PathIterator itTo = toPath.getShape()
				.getPathIterator(
					new AffineTransform() );
			...
			if (toPath != null)
			{
				PathIterator itFrom = fromPath.getShape()
					.getPathIterator(
						new AffineTransform() );
				PathIterator itTo = toPath.getShape()
					.getPathIterator(
				...
				for (; !itFrom.isDone(); itFrom.next())
				{
					...
					switch (segFrom)
					{
						...
						case PathIterator.SEG_CUBICTO:
							midPath.curveTo(
								(float) ( coordsFrom[ 0 ] * ( 1 - interp ) + coordsTo[ 0 ]
									* interp ),
								(float) ( coordsFrom[ 1 ] * ( 1 - interp ) + coordsTo[ 1 ]
									* interp ),
								(float) ( coordsFrom[ 2 ] * ( 1 - interp ) + coordsTo[ 2 ]
									* interp ),
								(float) ( coordsFrom[ 3 ] * ( 1 - interp ) + coordsTo[ 3 ]
									* interp ),
								(float) ( coordsFrom[ 4 ] * ( 1 - interp ) + coordsTo[ 4 ]
									* interp ),
								(float) ( coordsFrom[ 5 ] * ( 1 - interp ) + coordsTo[ 5 ]
									* interp )
								);
							break;
						case PathIterator.SEG_LINETO:
	...
	protected void rebuild( AnimTimeParser animTimeParser ) throws SVGException
	{
		...
		if (getPres( sty.withName( "to" ) ))
		{
			...
			else
			{
				...
				{
					//Try path
					toPath.setStringValue( strn );// = this.buildPath(strn, GeneralPath.WIND_EVEN_ODD);
					dataType = DT_PATH;
				}
				else
					dataType = DT_COLOR;
		...
		if (getPres( sty.withName( "values" ) ))
		{
			String strn = sty.getStringValue();
			if (strn != null)
				valuesValue = XMLParseUtil.parseDoubleList( strn );
