...
final class RadialGradientPaintContext extends MultipleGradientPaintContext
{
		...
		throws NoninvertibleTransformException
	{
		...
		{ //clamp focus to radius
			...
			focusX = (float) ( SCALEBACK * radius * Math.cos( angle ) )
				+ centerX;

			focusY = (float) ( SCALEBACK * radius * Math.sin( angle ) )
				+ centerY;
		}
										...
										int w, int h )
	{
		...
		final float constX = ( a00 * ( x - .5f ) ) + ( a01 * ( y + .5f ) )
			+ a02;
		final float constY = ( a10 * ( x - .5f ) ) + ( a11 * ( y + .5f ) )
			+ a12;
		float X; // User space point X coordinate 
		float Y; // User space point Y coordinate
		int i, j; //indexing variables for FOR loops
		...
		double pixSzSq = (float) ( a00 * a00 + a01 * a01 + a10 * a10 + a11
			* a11 );
