...
final class LinearGradientPaintContext extends MultipleGradientPaintContext
{
		...
		throws NoninvertibleTransformException
	{
		...
		Object colorRend = hints == null ? RenderingHints.VALUE_COLOR_RENDER_SPEED
			...
			hasDiscontinuity)
		{
			...
			if (colorRend == RenderingHints.VALUE_COLOR_RENDER_SPEED)
				...
				if (colorRend == RenderingHints.VALUE_COLOR_RENDER_QUALITY)
					fillMethod = ANTI_ALIAS_IMPL;
		}
	}
			...
			else
			{
				...
				if (dgdX > 0)
				{
					...
					while (off < gradLimit)
					{
						...
						int steps =
							(int) Math.ceil( ( fractions[ gradIdx + 1 ] - g )
								/ dgdX );
		...
		if (fillMethod == ANTI_ALIAS_IMPL)
		{
			...
			{ //for every row
				...
				{ //for every pixel in this row.
					//get the color
					pixels[ off++ ] = indexGradientAntiAlias(
						g,
						pixSz );
