...
public class AnimateTransform extends AnimateXform
{
		...
		SVGElement parent ) throws SAXException
	{
		...
		if (keyTimeStrn != null && valuesStrn != null)
		{
			...
			String[] valueList = Pattern.compile(
				";" )
				.split(
					valuesStrn );
		...
		if (additive != null)
		{
			if (additive.equals( "sum" ))
				this.additive = AT_SUM;
		}
	}
	...
	public AffineTransform eval( AffineTransform xform, double interp )
	{
		...
		switch (xformType)
		{
			case TR_TRANSLATE:
			{
				double x = ( 1.0 - interp ) * fromValue[ 0 ] + interp
					* toValue[ 0 ];
				double y = ( 1.0 - interp ) * fromValue[ 1 ] + interp
					* toValue[ 1 ];
			...
			case TR_ROTATE:
			{
				...
				double theta = ( 1.0 - interp ) * fromValue[ 0 ] + interp
					* toValue[ 0 ];
			...
			case TR_SKEWY:
			{
				double y = ( 1.0 - interp ) * fromValue[ 0 ] + interp
					* toValue[ 0 ];
				xform.setToShear(
					0.0,
					Math.toRadians( y ) );
	...
	protected void rebuild( AnimTimeParser animTimeParser ) throws SVGException
	{
		...
		if (getPres( sty.withName( "keyTimes" ) ))
		{
		...
		if (getPres( sty.withName( "additive" ) ))
		{
			String strn = sty.getStringValue()
				.toLowerCase();
			if (strn.equals( "sum" ))
