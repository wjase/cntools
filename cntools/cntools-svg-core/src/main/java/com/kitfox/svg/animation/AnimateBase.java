...
abstract public class AnimateBase extends AnimationElement
{
	...
	public void evalParametric( AnimationTimeEval state, double curTime )
	{
		evalParametric(
			...
			curTime,
			repeatCount,
			repeatDur == null ? Double.NaN : repeatDur.evalTime() );
	}
		...
		SVGElement parent ) throws SAXException
	{
		...
		if (strn == null)
		{
			...
			else
			{
				try
				{
					repeatCount = Double.parseDouble( strn );
				}
				catch (Exception e)
				{
					repeatCount = Double.NaN;
				}
			}
	}
	...
	protected void rebuild( AnimTimeParser animTimeParser ) throws SVGException
	{
		...
		if (getPres( sty.withName( "repeatDur" ) ))
		{
		...
		if (getPres( sty.withName( "repeatCount" ) ))
		{
			String strn = sty.getStringValue();
			if (strn == null)
			{
