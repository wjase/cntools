...
{
	static final Matcher matchPoint = Pattern.compile(
		"\\s*(\\d+)[^\\d]+(\\d+)\\s*" )
		.matcher(
			"" );
	...
	Path2DAttribute path = new Path2DAttribute( "path" );
	//GeneralPath path;
		...
		SVGElement parent ) throws SAXException
	{
		...
		if (path != null)
		{
			this.path.setStringValue( path );// = buildPath(path, GeneralPath.WIND_NON_ZERO);
		}
	...
	private void paramaterizePath()
	{
		...
		for (PathIterator pathIt = path.getShape()
			.getPathIterator(
	...
	protected void rebuild( AnimTimeParser animTimeParser ) throws SVGException
	{
		...
		if (getPres( sty.withName( "path" ) ))
		{
			String strn = sty.getStringValue();
			this.path.setStringValue( strn );// = buildPath(strn, GeneralPath.WIND_NON_ZERO);
		}

		if (getPres( sty.withName( "rotate" ) ))
		{
			String strn = sty.getStringValue();
			if (strn.equals( "auto" ))
			{
