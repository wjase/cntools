...
abstract class MultipleGradientPaintContext implements PaintContext
{
		...
		throws NoninvertibleTransformException
	{
		...
		model = GraphicsUtil.coerceColorModel(
			dataModel,
		...
		Color[] hiColors )
	{
		...
		if (( transparencyTest >>> 24 ) == 0xff)
		{
			if (dataModel.getColorSpace() == lrgbmodel_NA.getColorSpace())
				...
				if (dataModel.getColorSpace() == srgbmodel_NA.getColorSpace())
	...
	protected final int indexIntoGradientsArrays( float position )
	{
		...
		if (cycleMethod == MultipleGradientPaint.NO_CYCLE)
		{
			...
			if (cycleMethod == MultipleGradientPaint.REPEAT)
			{
				...
				( ( ( ( ( c1 << 16 ) & 0xFF0000 ) + ( ( ( ( c2 ) & 0xFF ) - ( ( c1 ) & 0xFF ) ) * w ) ) & 0xFF0000 ) >> 16 ) );
									...
									float sz, float weight )
	{
		...
		else
		{
			...
			if (DEBUG)
				System.out.println( "I1: " + i1 + " Idx1: " + idx1 +
											" I2: " + i2 + " Idx2: " + idx2 );
			...
			bch = ( bch + 0x08 ) >> 4;
			if (DEBUG)
				System.out.println( "Pix: [" + ach + ", " + rch +
											", " + gch + ", " + bch + "]" );
		}
	 ...
	 * large.
	 */
	protected final static synchronized WritableRaster getCachedRaster( ColorModel cm,
		int w,
		int h )
	{
		if (cm == cachedModel)
		...
		if (h < 32)
			h = 32;
		return cm.createCompatibleWritableRaster(
			w,
			h );
	}
