...
import com.kitfox.svg.elements.Marker.MarkerLayout;
import com.kitfox.svg.elements.Marker.MarkerPos;
...
abstract public class ShapeElement extends RenderableElement
{
		...
		throws SVGException
	{
		...
		//        if (getStyle(styleAttrib.withName("fill")) && getShape().contains(point))
	...
	@Override
	public void pick( Rectangle2D pickArea,
		AffineTransform ltw,
		boolean boundingBox,
	...
	protected void renderShape( Graphics2D g, Shape shape ) throws SVGException
	{
		...
		if (getStyle( styleAttrib.withName( "display" ) ))
		{
			if (styleAttrib.getStringValue()
				.equals(
					"none" ))
			{
				return;
		...
		if (getStyle( styleAttrib.withName( "fill" ) ))
		{
			if (styleAttrib.getStringValue()
				.equals(
					"none" ))
				...
				if (fillPaint == null)
				{
					...
					if (uri != null)
					{
						...
						SVGElement ele = diagram.getUniverse()
							.getElement(
								uri );
						fillPaint = ( (FillElement) ele ).getPaint(
							bounds,
							xform );
					}
				}
		...
		if (getStyle( styleAttrib.withName( "stroke" ) ))
		{
			...
			else
			{
				...
				if (strokePaint == null)
				{
					...
					{
						Rectangle2D bounds = shape.getBounds2D();
						AffineTransform shapeXform = g.getTransform();

						SVGElement ele = diagram.getUniverse()
							.getElement(
								uri );
						strokePaint = ( (FillElement) ele ).getPaint(
							bounds,
							shapeXform );
					}
				}
		...
		if (getStyle( styleAttrib.withName( "stroke-dasharray" ) ))
		{
		...
		if (getStyle( styleAttrib.withName( "stroke-dashoffset" ) ))
		{
			strokeDashOffset = styleAttrib.getFloatValueWithUnits();
		}

		int strokeLinecap = BasicStroke.CAP_BUTT;
		if (getStyle( styleAttrib.withName( "stroke-linecap" ) ))
		{
		...
		if (getStyle( styleAttrib.withName( "stroke-linejoin" ) ))
		{
			String val = styleAttrib.getStringValue();
			if (val.equals( "round" ))
			{
		...
		if (getStyle( styleAttrib.withName( "stroke-miterlimit" ) ))
		{
			strokeMiterLimit = Math.max(
				styleAttrib.getFloatValueWithUnits(),
				1 );
		}

		float strokeOpacity = opacity;
		if (getStyle( styleAttrib.withName( "stroke-opacity" ) ))
		{
			strokeOpacity *= styleAttrib.getRatioValue();
		}

		float strokeWidth = 1f;
		if (getStyle( styleAttrib.withName( "stroke-width" ) ))
		{
		...
		if (getStyle( styleAttrib.withName( "marker-start" ) )
			&& ( !styleAttrib.getStringValue()
				.equals(
					"none" ) ))
		{
			URI uri = styleAttrib.getURIValue( getXMLBase() );
			markerStart = (Marker) diagram.getUniverse()
				.getElement(
					uri );
		}

		Marker markerMid = null;
		if (getStyle( styleAttrib.withName( "marker-mid" ) )
			&& ( !styleAttrib.getStringValue()
				.equals(
					"none" ) ))
		{
			URI uri = styleAttrib.getURIValue( getXMLBase() );
			markerMid = (Marker) diagram.getUniverse()
				.getElement(
					uri );
		}

		Marker markerEnd = null;
		if (getStyle( styleAttrib.withName( "marker-end" ) )
			&& ( !styleAttrib.getStringValue()
				.equals(
					"none" ) ))
		{
			URI uri = styleAttrib.getURIValue( getXMLBase() );
			markerEnd = (Marker) diagram.getUniverse()
				.getElement(
					uri );
		}
		...
		if (fillPaint != null && fillOpacity != 0f)
		{
			if (fillOpacity <= 0)
			{
				...
				if (fillOpacity < 1f)
				{
					...
					if (cachedComposite instanceof AlphaComposite)
					{
						currentOpacity = ( (AlphaComposite) cachedComposite ).getAlpha();
					}
