...
public class SVGIcon implements Icon
{
	...
	{
		boolean old = this.preserveAspect;
	 ...
	 *
	 * @param comp - Component to draw icon to. This is ignored by SVGIcon, and can be set to null; only gg is used for
	 * drawing the icon
	...
	public void paintIcon( Component comp, Graphics gg, int x, int y )
	{
		...
		Object oldAliasHint = null;
		Object oldInterpolationHint = null;
		...
		try
		{
			...
			if (clipToViewbox)
			{
				g.setClip( new Rectangle2D.Float( 0,
					0,
					diagram.getWidth() - 1,
					diagram.getHeight() ) );
			}
			...
			if (comp != null) //&& preserveAspect
			{
				...
				{
					Insets i = ( (JComponent) comp ).getInsets();
					if (i != null)
					{
						width -= ( i.left + i.right );
						height -= ( i.top - i.right );
					}
				}
	...
	public void setSvgURI( URI svgURI )
	{
		...
		if (diagram != null)
		{
			...
			if (size == null)
			{
				size = new Dimension( (int) diagram.getRoot()
					.getDeviceWidth(), (int) diagram.getRoot()
					.
						getDeviceHeight() );
			}
			diagram.setDeviceViewport( new Rectangle( 0,
				0,
				size.width,
				size.height ) );
		}
	...
	public void setSvgResourcePath( String resourcePath )
	{
		...
		try
		{
			svgURI = new URI( getClass().getResource(
				resourcePath )
				.toString() );
			changes.firePropertyChange(
				"svgURI",
	...
	/**
	 * clipToViewbox will set a clip box equivalent to the SVG's viewbox before
	 * rendering.
	 */
