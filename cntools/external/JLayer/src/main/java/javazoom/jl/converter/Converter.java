...
public class Converter
{
		...
		throws JavaLayerException
	{
		...
		try
		{
			...
			progressListener.converterUpdate(
				ProgressListener.UPDATE_FRAME_COUNT,
				frameCount,
				0 );
				...
				for (; frame < frameCount; frame++)
				{
					try
					{
						...
						if (output == null)
						{
							...
							int channels = ( header.mode() == Header.SINGLE_CHANNEL ) ? 1
								: 2;
							int freq = header.frequency();
							output = new WaveFileObuffer( channels,
								freq,
								destName );
					...
					catch (Exception ex)
					{
						...
						if (stop)
						{
							throw new JavaLayerException( ex.getLocalizedMessage(),
								ex );
						}
					}
				}
			...
			progressListener.converterUpdate(
				ProgressListener.UPDATE_CONVERT_COMPLETE,
				time,
				frame );
		}
			...
			if (isDetail( VERBOSE_DETAIL ))
			{
				switch (updateID)
				{
					case UPDATE_CONVERT_COMPLETE:
						...
						pw.println();
						pw.println( "Converted " + param2 + " frames in "
							+ param1 + " ms (" +
