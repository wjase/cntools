...
class LayerIDecoder implements FrameDecoder
{
	...
	protected void readSampleData()
	{
		...
		do
		{
			...
			{
				for (i = 0; i < num_subbands; ++i)
					write_ready = subbands[ i ].put_next_sample(
						which_channels,
						filter1,
						filter2 );
	...
	static abstract class Subband
	{
		...
		public abstract void read_allocation( Bitstream stream,
			Header header,
	...
	static class SubbandLayer1 extends Subband
	{
			...
			throws DecoderException
		{
			if (( allocation = stream.get_bits( 4 ) ) == 15)
			{
				...
				throw new DecoderException( DecoderErrors.ILLEGAL_SUBBAND_ALLOCATION,
					null );
		...
		{
			if (( allocation != 0 )
				&& ( channels != OutputChannels.RIGHT_CHANNEL ))
			{
				float scaled_sample = ( sample * factor + offset )
					* scalefactor;
				filter1.input_sample(
					scaled_sample,
					subbandnumber );
			}
			return true;
