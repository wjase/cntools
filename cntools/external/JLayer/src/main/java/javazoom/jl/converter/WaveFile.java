/*
 * 11/19/04 1.0 moved to LGPL. 02/23/99 JavaConversion by E.B Don Cross, April 1993. RIFF file
 * format classes. See Chapter 8 of "Multimedia Programmer's Reference" in the Microsoft Windows
 * SDK.
 * 
...
public class WaveFile extends RiffFile
{
	...
	class WaveFormat_ChunkData
	{
		...
		public void Config( int NewSamplingRate,
			short NewBitsPerSample,
			short NewNumChannels )
		{
		...
		if (retcode == DDC_SUCCESS)
		{
			...
			if (retcode == DDC_SUCCESS)
			{
				...
				retcode = Write(
					...
					8 );
				retcode = Write(
					wave_format.data.wFormatTag,
					2 );
				retcode = Write(
					wave_format.data.nChannels,
					2 );
				retcode = Write(
					wave_format.data.nSamplesPerSec,
					4 );
				retcode = Write(
					wave_format.data.nAvgBytesPerSec,
					4 );
				retcode = Write(
					wave_format.data.nBlockAlign,
					2 );
				retcode = Write(
					wave_format.data.nBitsPerSample,
					2 );
				/* byte[] br = new byte[16];
