...
final class SynthesisFilter
{
	...
	private void compute_new_v_old()
	{
		...
		// this is pretty insane coding
		float tmp1;
		new_v[ 36 - 17 ] = -( new_v[ 4 ] = ( new_v[ 12 ] = p[ 7 ] ) + p[ 5 ] )
			- p[ 6 ];
		new_v[ 44 - 17 ] = -p[ 6 ] - p[ 7 ] - p[ 4 ];
		new_v[ 6 ] = ( new_v[ 10 ] = ( new_v[ 14 ] = p[ 15 ] ) + p[ 11 ] )
			+ p[ 13 ];
		new_v[ 34 - 17 ] = -( new_v[ 2 ] = p[ 15 ] + p[ 13 ] + p[ 9 ] )
			- p[ 14 ];
		new_v[ 38 - 17 ] = ( tmp1 = -p[ 14 ] - p[ 15 ] - p[ 10 ] - p[ 11 ] )
			- p[ 13 ];
		...
		// coding like this is hard to read
		float tmp2;
		new_v[ 5 ] = ( new_v[ 11 ] = ( new_v[ 13 ] = ( new_v[ 15 ] = p[ 15 ] )
			+ p[ 7 ] )
			+ p[ 11 ] )
							+ p[ 5 ] + p[ 13 ];
		new_v[ 7 ] = ( new_v[ 9 ] = p[ 15 ] + p[ 11 ] + p[ 3 ] ) + p[ 13 ];
		new_v[ 33 - 17 ] = -( new_v[ 1 ] = ( tmp1 = p[ 13 ] + p[ 15 ] + p[ 9 ] )
			+ p[ 1 ] )
			- p[ 14 ];
		new_v[ 35 - 17 ] = -( new_v[ 3 ] = tmp1 + p[ 5 ] + p[ 7 ] ) - p[ 6 ]
			- p[ 14 ];
		...
		new_v[ 47 - 17 ] = ( tmp1 = -p[ 8 ] - p[ 12 ] - p[ 14 ] - p[ 15 ] )
			- p[ 0 ];
	...
	static private float[] load_d()
	{
		try
		{
			...
			Object o = JavaLayerUtils.deserializeArrayResource(
				"sfd.ser",
				elemType,
				512 );
			return (float[]) o;
	 ...
	 */
	static private float[] subArray( final float[] array,
		final int offs,
		int len )
	{
